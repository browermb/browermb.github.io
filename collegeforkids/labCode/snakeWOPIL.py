...
class Master(Canvas):
    ...
    def start(self):
        """start snake game"""
        #COMPLETE
        #write code to make this happen only if self.running is zero
    ...
    def clean(self):
        """restarting the game"""
        #COMPLETE
        #write code to make this happen only if self.running is one
            self.score.reset()
            self.current.stop()
            self.running = 0
...
class Shape:
    ...
    def __init__(self, can, a, b, kind):
        self.can = can
        #COMPLETE
        #set self.x value to a 
        #set self.y value to b
        self.kind = kind


        """There are two kinds: OB (obstacle) and SN (snake)"""
        #COMPLETE
        #Make the following code only happen if kind is snake
            self.ref = Canvas.create_rectangle(self.can,
                                               ...
                                               width=2)
        #COMPLETE
        #Make the following code only happen if kind is obstacle
...
class Snake:
    ...
    def move(self, path):
        ...
        #COMPLETE
        """Write code so that the code only happens if a is equal to self.can.obstacle.x 
            AND if b is equal to self.can.obstacle.y"""
...
class Movement:
    ...
    def begin(self):
        ...
        #COMPLETE
        #Write an if statement so that the code only happens if self.flag is greater than 0
